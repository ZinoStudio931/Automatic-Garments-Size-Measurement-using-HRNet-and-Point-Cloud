...
def train(config, train_loader, train_dataset, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            # Debug # 01) 
            # trial # 01) 
            # Using 'preds_local' before assining - issue
            # -> 'preds' instead of 'preds_local' (maybe?) since in ln 61~81, 
            # 'preds_local' only using for making 'preds' in else clause (hmmm...)

            # save_debug_images(config, input, meta, target, preds_local, output, prefix)
            save_debug_images(config, input, meta, target, preds, output, prefix) # Debug # 01) trial # 01 ) 
...
def validate(config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    num_samples = len(val_dataset) # for debug 64 images -> 107 == len(val_dataset.db) 
    all_preds = np.zeros(
        (num_samples, config.MODEL.NUM_JOINTS, 3), # (107, 294, 3) == (Items_num, LM_num, )
        dtype=np.float32
    )
    all_boxes = np.zeros((num_samples, 7)) # (107, 7) 
    image_path = []
    filenames = []
    imgnums = []
    idx = 0
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader): # len(val_loader)==14
            # input.shape >>> torch.Size([2, 3, 384, 288])
            ...
            channel_mask = torch.zeros_like(target_weight).float()
            for j, cat_id in enumerate(cat_ids): # tensor([8, 1])
                # if 'cat_id' == 1 
                rg = val_dataset.gt_class_keypoints_dict[int(cat_id)] # rg = (0, 25) 
                index = torch.tensor(
                    [list(range(rg[0], rg[1]))], # [list(range(0, 25))] >>> [[0, 1, 2, 3, 4, 5, 6, 7, 8, ...]]
            ...
            # *** input.shape >>> torch.Size([2, 3, 384, 288])
            output = model(input) 
            # *** output.shape >>> torch.Size([2, 294, 96, 72])
            ...
            if config.MODEL.TARGET_TYPE == 'gaussian':
                ...
                # preds_local (2, 294, 2), maxvals (2, 294, 1)
                # Transform back from heatmap coordinate to image coordinate
                preds = preds_local.copy()
                for i in range(preds_local.shape[0]):
                    preds[i] = transform_preds(
            ...
            num_images = input.size(0) # 8 == input.shape[0]
            ...
            if i % config.PRINT_FREQ == 0:
                ...
                save_debug_images(config, input, meta, target, preds_local, output, prefix)
        # *** Scoring part + Save annot file too 
        ...
        if isinstance(name_values, list):
            ...
            _print_name_value(name_values, model_name) # - (2)
        # - (2)
        # | Arch | AP | Ap .5 | AP .75 | AP (M) | AP (L) | AR | AR .5 | AR .75 | AR (M) | AR (L) |
        # |---|---|---|---|---|---|---|---|---|---|---|
        # | pose_hrnet | 0.7017 | 0.9564 | 0.8011 | 0.5787 | 0.7033 | 0.7395 | 0.9653 | 0.8272 | 0.5920 | 0.7413 |
